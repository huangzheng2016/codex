...
#[clap(
    ...
    // `codex-sec-x86_64-unknown-linux-musl`, but the help output should always
    // use the generic `codex-sec` command name that users run.
    bin_name = "codex-sec",
    override_usage = "codex-sec [OPTIONS] [PROMPT]\n       codex-sec [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
